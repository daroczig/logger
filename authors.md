# Authors and Citation

## Authors

- **Gergely Daróczi**. Author, maintainer.
  [](https://orcid.org/0000-0003-3149-8537)

- **Hadley Wickham**. Author. [](https://orcid.org/0000-0003-4757-117X)

- **Jonathan Carroll**. Contributor.
  [](https://orcid.org/0000-0002-1404-5264)

- **[![](https://sparecores.com/assets/images/logos/sc-logo-horizontal-transparent-for-light-theme.png)](https://sparecores.com)**.
  Funder.

- **[![](https://cdn.system1.com/s1c/dist/img/system1-logo.svg)](https://system1.com)**.
  Funder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/daroczig/logger/blob/v0.4.2/DESCRIPTION)

Daróczi G, Wickham H (2026). *logger: A Lightweight, Modern and Flexible
Logging Utility*. R package version 0.4.2,
<https://daroczig.github.io/logger/>.

    @Manual{,
      title = {logger: A Lightweight, Modern and Flexible Logging Utility},
      author = {Gergely Daróczi and Hadley Wickham},
      year = {2026},
      note = {R package version 0.4.2},
      url = {https://daroczig.github.io/logger/},
    }
