...
While `logger` has a `log_` prefix for all logging functions, `log4r` used to have lowercase functions names referring to the log level, which takes a logging object and the log message, but since version 0.5, `log4r` also follows the `log_` prefix format:
...
Since version 0.5, the `log4r` package also follows the `appenders` and layout formatter functions, similar to `logger`.
