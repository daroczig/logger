...
As you can see, `futile.logger` provided a bit more information about the log request in a structured way, while `logger` simply used the default parameters, but it's easy to extend the latter with the calling function name, `pid` of the R session, name of the machine etc -- see `?get_logger_meta_variables` for a more complete list. `logger` also ships a but more layouts, eg `?layout_glue_colors` or roll out your own via the `?layout_glue_generator` factory function.
...
logwarn('... where the default log message formatter is %s', 'sprintf')
```
## loggit
