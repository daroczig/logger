...
<img src="logger_structure.svg" title="The structure of a logger and the flow of a log record request" alt="The structure of a logger and the flow of a log record request" style="width: 100%;" />
...
- a **logger definition** to process the log request, including
    ...
    - `layout` function, which takes log message(s) and further information on the log request (such as like timestamp, hostname, username, calling function etc) to render the actual log records eg human-readable text, JSON etc
