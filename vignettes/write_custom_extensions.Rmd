...
In this vignette I suppose that you are already familiar with `vignette("customize_logger")`.
...
Your easiest option to set up a custom layout is calling `?layout_glue_generator` that comes with a nice API being able to access a bunch of meta-information on the log request via `?get_logger_meta_variables`. For such example, see the `vignette("customize_logger")`.
