...
When writing a custom formatter function, it should also accept the original log request's environment as `.envir`, which can be used to find the relevant variables in your formatter.
...
- the namespace (as `namespace`), calling function (as `.call`) and environment (as `.envir`) of the log request automatically collected by `?log_level`
